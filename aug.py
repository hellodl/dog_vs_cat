...
def op_0(nb_ep, saveWeights=True, modelTest=True, aug=False):
    ...
    for epoch in range(nb_ep):
        ...
        conv_model.fit_generator(batches, steps_per_epoch=math.ceil(batches.samples / batches.batch_size),
                                 validation_data=val_batches,
                                 epochs=1,
                                 validation_steps=math.ceil(val_batches.samples / val_batches.batch_size), verbose=1)

        curTime = time.strftime(ISOTIMEFORMAT, time.localtime())
        latest_weights_filename = 'ft%d_%s.h5' % (epoch, curTime)
        if saveWeights is True:
            conv_model.save_weights(result_path + '/' + latest_weights_filename)

        if modelTest is True:
            if aug is True:
                preds = conv_model.predict_generator(test_batches,
                                                     steps=math.ceil(test_batches.samples / test_batches.batch_size),
                                                     verbose=1)
            else:
                pass
                # preds = conv_model.predict(tst_features, batch_size=100, verbose=1)
